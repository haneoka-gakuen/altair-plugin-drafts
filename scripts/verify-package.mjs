...
const policies = {
  "@haneoka/altair-plugin-drafts": {
    repository: "git+https://github.com/haneoka-gakuen/altair-plugin-drafts.git",
    peerDependencies: ["@haneoka/altair"],
    allowedImports: ["@haneoka/altair/plugins"],
...
if (manifest.altair?.kind !== "service") fail("altair.kind must be service");
...
if (!builtJavaScript.includes("haneoka.altair.drafts")) {
  fail("built canonical draft service id is missing");
}
if (builtJavaScript.includes('"altair.drafts"')) {
  fail("built package contains the obsolete draft service id");
}
