...
import { AltairDraftPersistenceError, normalizeAltairDraftPersistenceError } from "./persistence.js";
import {
...
export const ALTAIR_DRAFT_SERVICE = defineAltairService<AltairDraftService>(ALTAIR_DRAFT_SERVICE_ID);
...
class DraftSession implements AltairDraftSession {
  ...
  updateScene(update: AltairSceneDraftUpdate): Promise<void> {
    return this.#mutate("updating a scene draft", (state) => {
      ...
      const projectSnapshot = requireText(update.projectSnapshot, "scene project snapshot");
      ...
      const context =
        update.context === undefined
          ? previous?.context
          : copyJson(update.context, `scene ${JSON.stringify(sceneId)} context`);
  ...
  reconcileScene(sceneIdValue: string, baselineValue: string): Promise<AltairSceneDraftReconciliation> {
  ...
  reload(): Promise<AltairDraftSessionSnapshot> {
    return this.#enqueue(async () => {
      ...
      const loaded = await persistenceOperation("reloading a draft session", () => this.persistence!.load(this.name));
      const normalized =
        loaded === undefined
  ...
  async flush(): Promise<void> {
    ...
    if (this.persistence?.flush) {
      await persistenceOperation("flushing a draft session", () => this.persistence!.flush!());
    }
  }
  ...
  dispose(): Promise<void> {
    ...
    this.#disposePromise = (async () => {
      ...
      try {
        await this.#tail;
        if (this.persistence?.flush) {
          await persistenceOperation("flushing a disposed draft session", () => this.persistence!.flush!());
        }
      } catch (error) {
        failure = error;
      } finally {
  ...
  #mutate(operation: string, mutate: (state: DraftState) => void): Promise<void> {
    return this.#enqueue(async () => {
      const candidate = cloneState(this.#state);
      mutate(candidate);
    ...
    if (!this.#accepting || this.#disposed) {
      return Promise.reject(new Error(`Altair draft session ${JSON.stringify(this.name)} is disposed`));
    }
  ...
  #assertReadable(): void {
    if (this.#disposed) {
      throw new Error(`Altair draft session ${JSON.stringify(this.name)} is disposed`);
    }
  }
}
...
class DraftService implements AltairDraftService {
  ...
  constructor(options: AltairDraftServiceOptions) {
    this.maxSessions = boundedInteger(options.maxSessions, DEFAULT_MAX_SESSIONS, 1, 128, "maxSessions");
    ...
    this.#adapter = options.persistence === undefined ? undefined : requirePersistence(options.persistence);
    this.persistence =
  ...
  open(nameValue: string, seed?: AltairDraftSessionSeed): Promise<AltairDraftSession> {
    this.#assertActive();
    const name = requireSessionName(nameValue);
    if (this.#sessions.has(name) || this.#openingNames.has(name)) {
      return Promise.reject(new Error(`Altair draft session already exists: ${name}`));
    }
    if (this.#sessions.size + this.#openingNames.size >= this.maxSessions) {
      return Promise.reject(new Error(`Altair draft service reached its ${this.maxSessions} session limit`));
    }
  ...
  async flush(): Promise<void> {
    ...
    const results = await Promise.allSettled([
      ...
      ...(this.#adapter?.flush
        ? [persistenceOperation("flushing the draft service", () => this.#adapter!.flush!())]
        : []),
    ]);
  ...
  dispose(): Promise<void> {
    ...
    this.#disposePromise = (async () => {
      ...
      const sessions = await Promise.allSettled([...this.#sessions.values()].map((session) => session.dispose()));
      const adapter: PromiseSettledResult<unknown>[] = [];
      if (this.#adapter?.flush) {
        adapter.push(
          await settlePromise(
            persistenceOperation("flushing the disposed draft service", () => this.#adapter!.flush!()),
          ),
        );
      }
      if (this.#adapter?.dispose) {
        adapter.push(
          await settlePromise(persistenceOperation("disposing draft persistence", () => this.#adapter!.dispose!())),
        );
      }
      this.#sessions.clear();
      throwSettledFailures([...opening, ...sessions, ...adapter], "Failed to dispose Altair drafts");
    })();
    return this.#disposePromise;
  }

  async #openNow(name: string, fallbackSeed: AltairDraftSessionSeed | undefined): Promise<AltairDraftSession> {
    let state: DraftState;
    let revision: number | null = null;
    if (this.#adapter) {
      const loaded = await persistenceOperation("loading a draft session", () => this.#adapter!.load(name));
      if (loaded !== undefined) {
        const normalized = normalizePersistedSession(loaded, name, this.#limits);
        ...
        if (statePending(state)) {
          revision = await persistState(name, state, null, this.#adapter, "persisting an initial draft seed");
        }
      }
    } else {
      state = stateFromSeed(fallbackSeed);
    ...
    const session = new DraftSession(name, state, revision, this.#limits, this.#adapter, (disposed) => {
...
export const createAltairDraftService = (options: AltairDraftServiceOptions = {}): AltairDraftService =>
  new DraftService(options);

export const createAltairDraftsPlugin = (options: AltairDraftsPluginOptions = {}): AltairPluginV2 => {
  if (options.persistenceFactory !== undefined && typeof options.persistenceFactory !== "function") {
    throw new TypeError("Altair draft persistenceFactory must be a function");
  }
  const serviceOptions = Object.freeze({
    ...(options.maxSessions === undefined ? {} : { maxSessions: options.maxSessions }),
    ...(options.maxScenesPerSession === undefined ? {} : { maxScenesPerSession: options.maxScenesPerSession }),
  ...
  return defineAltairPlugin({
    manifest: {
      ...
      version: "0.1.0",
      apiVersion: 2,
      description: "Bounded, optionally persistent draft sessions for Altair editors",
...
const requirePersistence = (persistence: AltairDraftPersistence): AltairDraftPersistence => {
  if (!persistence || typeof persistence !== "object") {
    throw new TypeError("Altair draft persistence must be an object");
  }
  if (typeof persistence.id !== "string" || !persistence.id.trim() || persistence.id.length > 256) {
    throw new TypeError("Altair draft persistence id is invalid");
  }
  if (typeof persistence.durable !== "boolean") {
    throw new TypeError("Altair draft persistence durable flag must be boolean");
  }
  for (const method of ["load", "save", "remove"] as const) {
    if (typeof persistence[method] !== "function") {
      throw new TypeError(`Altair draft persistence ${method} must be a function`);
    }
  }
  for (const method of ["flush", "dispose"] as const) {
    if (persistence[method] !== undefined && typeof persistence[method] !== "function") {
      throw new TypeError(`Altair draft persistence ${method} must be a function`);
    }
  }
  return persistence;
...
const stateFromSeed = (seed: AltairDraftSessionSeed | undefined): DraftState => {
  ...
  for (const scene of seed?.scenes ?? []) {
    ...
    if (state.scenes.has(record.sceneId)) {
      throw new Error(`Altair draft seed repeats scene ${JSON.stringify(record.sceneId)}`);
    }
...
const cloneState = (state: DraftState): DraftState => stateFromSeed(seedFromState(state));

const seedFromState = (state: DraftState): AltairDraftSessionSeed =>
  freezeResult({
    ...(state.project === undefined ? {} : { project: snapshotDraftRecord(state.project) }),
...
const statePending = (state: DraftState): boolean => state.project !== undefined || state.scenes.size > 0;
...
const persistState = async (
  ...
  await persistenceOperation(operation, () => persistence.save(record, { expectedRevision }));
  return revision;
};
...
const normalizePersistedSession = (
  ...
  if (!value || typeof value !== "object") {
    throw new AltairDraftPersistenceError("invalid-data", "Altair persisted draft session must be an object");
  }
  if (value.format !== "altair-draft-session" || value.formatVersion !== 1) {
    throw new AltairDraftPersistenceError("invalid-data", "Altair persisted draft session has an unsupported format");
  }
...
const persistenceOperation = async <T>(operation: string, run: () => Promise<T>): Promise<T> => {
  try {
    return await run();
  } catch (error) {
...
const throwSettledFailures = (results: readonly PromiseSettledResult<unknown>[], message: string): void => {
  const failures = results.flatMap((result) => (result.status === "rejected" ? [result.reason] : []));
  if (failures.length === 1) throw failures[0];
  if (failures.length > 1) throw new AggregateError(failures, message);
};

const settlePromise = async (promise: Promise<unknown>): Promise<PromiseSettledResult<unknown>> =>
...
const reconciliation = (record: MutableSceneDraftRecord): AltairSceneDraftReconciliation =>
  freezeResult({
    sceneId: record.sceneId,
    value: record.value,
    pending: true,
    ...(record.context === undefined ? {} : { context: copyJson(record.context, "scene context") }),
    ...(record.base === undefined ? {} : { base: copyBase(record.base) }),
  });

const detectConflict = (
  state: DraftState,
  current: Required<Pick<AltairDraftCurrentProject, "snapshot" | "revision">> & {
    readonly nextSceneIds?: readonly string[];
  },
): AltairDraftConflict | undefined => {
  const records = [...(state.project === undefined ? [] : [state.project]), ...state.scenes.values()];
  if (records.length === 0) return undefined;
  if (records.some(({ base }) => base === undefined)) {
    return "missing-base";
  }
  if (records.some(({ base }) => base!.snapshot !== current.snapshot || base!.revision !== current.revision)) {
    return "project-changed";
  }
  if (current.nextSceneIds !== undefined) {
    const nextSceneIds = new Set(current.nextSceneIds);
    if ([...state.scenes.keys()].some((sceneId) => !nextSceneIds.has(sceneId))) {
...
const copyDraftRecord = (record: AltairDraftRecord, label: string): MutableDraftRecord => {
...
const copySceneDraftRecord = (record: AltairSceneDraftRecord): MutableSceneDraftRecord => {
  ...
  return {
    ...
    ...(record.context === undefined
      ? {}
      : {
          context: copyJson(record.context, `scene ${JSON.stringify(sceneId)} context`),
        }),
  };
};
...
const snapshotSceneDraftRecord = (record: MutableSceneDraftRecord): AltairSceneDraftRecord =>
  freezeResult({
    sceneId: record.sceneId,
    value: record.value,
    ...(record.base === undefined ? {} : { base: copyBase(record.base) }),
...
const copyBase = (base: AltairDraftBase): AltairDraftBase => {
  ...
  return frozenBase(requireText(base.snapshot, "draft base snapshot"), requireRevision(base.revision));
};

const frozenBase = (snapshot: string, revision: number): AltairDraftBase => Object.freeze({ snapshot, revision });
...
const requireRevision = (value: number): number => {
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new TypeError("Altair draft revision must be a non-negative safe integer");
  }
  return value;
};
...
const requireSessionName = (value: string): string => {
  ...
  if (name.length === 0 || name.length > MAX_SESSION_NAME_LENGTH || /[\u0000-\u001f\u007f]/u.test(name)) {
    throw new TypeError(`Altair draft session name must contain 1-${MAX_SESSION_NAME_LENGTH} visible characters`);
  }
  return name;
};
...
const requireSceneId = (value: string): string => {
  ...
  if (sceneId.length === 0 || sceneId.length > MAX_SCENE_ID_LENGTH || /[\u0000-\u001f\u007f]/u.test(sceneId)) {
    throw new TypeError(`Altair scene id must contain 1-${MAX_SCENE_ID_LENGTH} visible characters`);
  }
  return sceneId;
};
...
const boundedInteger = (
  ...
  if (!Number.isSafeInteger(result) || result < minimum || result > maximum) {
    throw new RangeError(`Altair ${label} must be an integer from ${minimum} to ${maximum}`);
  }
  return result;
};
...
const assertWithinLimits = (name: string, state: DraftState, limits: DraftLimits): void => {
  if (state.scenes.size > limits.maxScenesPerSession) {
    throw new Error(`Altair draft session ${JSON.stringify(name)} exceeds its scene limit`);
  }
  const bytes = sessionBytes(state);
  if (bytes > limits.maxSessionBytes) {
    throw new Error(`Altair draft session ${JSON.stringify(name)} exceeds its ${limits.maxSessionBytes}-byte limit`);
  }
};
...
const sessionBytes = (state: DraftState): number => {
  ...
  const recordBytes = (record: MutableDraftRecord): number =>
    textBytes(record.value) + (record.base === undefined ? 0 : textBytes(record.base.snapshot) + 8);
